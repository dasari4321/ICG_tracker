...
parser.add_argument('--dataset', default='VOT2018',type=str,
        help='datasets')
parser.add_argument('--video', default='wiper', type=str,
        help='eval one special video') #Bird1 #Board
parser.add_argument('--vis', action='store_true',
        help='whether visualzie result')
parser.add_argument('--name', default='eco_', type=str,
...
#edg edg_res hog hog_res res res_edg_hog
...
####################    # create model
    ...
    args.name = args.tname +'_'+ args.tparam
####################
    ...
    if args.dataset in ['VOT2016', 'VOT2018', 'VOT2019']:
        ...
        for v_idx, video in enumerate(dataset):
            ...
            scores = []
            pred_bboxes = []
            txt_path = '/media/hd2/mohana/pytracking/pysot_toolkit/results/VOT2018/eco_ft_edg_res/baseline/'+video.name
            box_path = os.path.join(txt_path, '{}_001.txt'.format(video.name))
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if idx == frame_counter:
                    ...
                    vis = vis_bb[idx].split(',')
                    if len(vis)>1:
                        vis = [int(float(x)) for x in vis if len(vis)>1 ]
                        vis_pr = vis
                    else:
                        vis = [int(float(x)) for x in vis_pr if len(vis_pr)>1]
                ...
                if args.vis and idx > frame_counter:
                    ...
                    cv2.rectangle(img, (vis[0], vis[1]),
                                  (vis[0]+vis[2], vis[1]+vis[3]), (255, 0, 0), 3)

                    cv2.putText(img, str(idx), (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
#                    cv2.putText(img, str(lost_number), (40, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                    cv2.imshow(video.name, img)
                    cv2.imwrite('./visual/{}/{:d}.jpg'.format(video.name,idx), img)
