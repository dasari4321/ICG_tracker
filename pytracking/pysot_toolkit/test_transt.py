...
def main(mode='visible', visulization=False):
    assert mode in ['infrared', 'visible'], 'Only Support IR or RGB to evalute'
    # setup tracker
    net_path = './transt.pth'#'./ep600.pth.tar'
    ...
    # setup experiments
    video_paths = sorted(glob.glob(os.path.join('/home/dasari/datasets/Anti-UAV-RGBT','test', '*')))
    ...
    for video_id, video_path in enumerate(video_paths, start=1):
        ...
        res_file = os.path.join(video_path, '%s.json'%mode)
        ...
        while True:
            ...
            if frame_id == 0:
                ...
                info = { }
                outputs = tracker.track(frame, info)
...
if __name__ == '__main__':
    main(mode='visible', visulization=False)
