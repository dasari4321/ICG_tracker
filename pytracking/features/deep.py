...
class cvt(MultiFeatureBase):
    ...
    def __init__(self, output_layers, net_path=None, use_gpu=True, *args, **kwargs):
        super(cvt, self).__init__(*args, **kwargs)

        for l in output_layers:
            if l not in ['layer1', 'layer2', 'layer3','clss']:
                raise ValueError('Unknown layer')

        self.output_layers = list(output_layers)
        self.use_gpu = use_gpu
        self.net_path = 'cvt/cvt.pth.tar' if net_path is None else net_path
    ...
    def initialize(self):
        ...
        self.layer_stride = {'layer1': 4, 'layer2': 8, 'layer3': 16,'clss': None}
        self.layer_dim = {'layer1': 64, 'layer2': 192, 'layer3': 384, 'clss': None}
    ...
    def extract(self, im: torch.Tensor):
        ...
        if self.use_gpu:
            im = im.cuda()
        with torch.no_grad():
            ft1 = self.net.feat_layers[0](im)
            ft2 = self.net.feat_layers[1](ft1)
            ft3 = self.net.feat_layers[2](ft2)

            return TensorList([ft1, ft3])
...
class ResNet18m1(MultiFeatureBase):
    ...
    def __init__(self, output_layers, net_path=None, use_gpu=True, *args, **kwargs):
        ...
        for l in output_layers:
            if l not in ['vggconv1', 'conv1', 'layer1', 'layer2', 'layer3', 'layer4', 'fc','edg']:
                raise ValueError('Unknown layer')

        self.output_layers = list(output_layers)
        self.use_gpu = use_gpu
        self.net_path = "/media/hd2/mohana/pytracking/pytracking/networks/resnet18_vggmconv1/resnet18_vggmconv1.pth" if net_path is None else net_path
    def initialize(self):
...
#        super(ResNet18m1, self).__init__(*args, **kwargs)
        ...
        for l in output_layers:
            if l not in ['init_conv','layer1', 'layer2', 'layer3', 'layer4', 'layer5','layer6','layer_out', 'edg', 'ft']:
                raise ValueError('Unknown layer')

        self.output_layers = list(output_layers)
        self.use_gpu = use_gpu
        self.net_path = '/media/hd2/mohana/pytracking/pytracking/networks/mobile/mobilenet_large.t7' if net_path is None else net_path
    ...
    def initialize(self):
        ...
        self.layer_stride = {'init_conv': 2, 'layer1': 2, 'layer2': 4, 'layer3': 8, 'layer4': 16, 'layer5': 16,'layer6': 32, 'layer_out': 32, 'edg':1}
        self.layer_dim = {'init_conv': 16, 'layer1': 16, 'layer2': 24, 'layer3': 40, 'layer4': 80, 'layer5': 112,'layer6': 160, 'layer_out': 960, 'edg':21}
        ...
        for net_path in net_path_full:
            try:
                self.net = mobilenet3(self.output_layers)
                break
            except:
                pass
        if self.net is None:
